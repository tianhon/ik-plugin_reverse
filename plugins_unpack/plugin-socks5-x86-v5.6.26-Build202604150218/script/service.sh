...
create_custom_iprules() {
    ...
    while true; do
        ...
        if [ "$dnsmode" = "interdns" ] || [ "$dnsmode" = "interdnspro" ]; then
            ...
            if [ "$dnsmode" = "interdnspro" ]; then
                sensitive_domains=$(jq -r '.sensitive_domains // [] | join(" ")' "$RESOURCE_CACHE_FILE" 2>/dev/null)
                [ -z "$sensitive_domains" ] && sensitive_domains="google youtube tiktok facebook twitter openai claude anthropic whatsapp telegram instagram twimg akamaihd"
...
import_servers() {
    ...
    if [ "$type" == "others" ]; then
      ...
      ' $CRASHDIR/configs/server.tmp >"$batch_file"
    elif [ "$type" == "vlessvmess" ]; then
        suffix="node"
        [ -z "$isAdv" ] && isAdv=$(authtool check-plugin 6 >/dev/null 2>&1 && echo "true" || echo "false")
        if [ "$isAdv" != "true" ]; then
            echo "需要高级版授权"
            return 1
        fi
        ...
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            if [[ "$line" == "vless|"* ]]; then
                IFS='|' read -r _ uuid server port transport security pbk fp sni sid flow name_node path host alpn allowInsecure <<<"$line"
                name_node=$(echo "$name_node" | sed -e 's/[ |]/-/g' -e 's/ //g' -e 's/-\+/-/g')
                [ -z "$name_node" -a "$autoNodeName" != "true" ] && continue
                [ "$autoNodeName" == "true" ] && name_node="iKuai_vless-${suffix}${index}"
                name_node="iKuai_${name_node#iKuai_}"

                yaml_config="name: $name_node, server: $server, port: $port, uuid: $uuid, type: vless, network: $transport, security: $security, "
                if [ "$security" != "none" ]; then
                    yaml_config+="tls: true, client-fingerprint: ${fp:-chrome}, "
                    [ -n "$sni" ] && yaml_config+="servername: $sni, "
                fi
                if [ "$security" == "reality" ] && [ -n "$pbk" ]; then
                    yaml_config+="reality-opts: {public-key: $pbk, short-id: $sid}, "
                    [ -n "$flow" ] && yaml_config+="flow: $flow, "
                fi
                if [ "$transport" == "ws" ] && [ -n "$path" ]; then
                    # path已经在前端处理过？参考脚本有url解码。这里简单处理。
                    yaml_config+="ws-opts: {path: $path, headers: {Host: $host}}, "
                fi
                [ -n "$alpn" ] && yaml_config+="alpn: [$(echo "$alpn" | sed 's/,/","/g' | awk '{print "\""$0"\""}')], "
                [ "$allowInsecure" == "1" ] && yaml_config+="skip-cert-verify: true, "
                [ -n "$group" ] && yaml_config+="ik_group: $group, "
                yaml_config+="udp: true, tfo: $tfoEnable"
                printf "name:%s,|- {%s}\n" "$name_node" "$yaml_config" >>"$batch_file"
                index=$((index + 1))
            elif [[ "$line" == "vmess|"* ]]; then
                IFS='|' read -r _ uuid server port cipher alterId net tls name_node wsPath wsHost <<<"$line"
                name_node=$(echo "$name_node" | sed -e 's/[ |]/-/g' -e 's/ //g' -e 's/-\+/-/g')
                [ -z "$name_node" -a "$autoNodeName" != "true" ] && continue
                [ "$autoNodeName" == "true" ] && name_node="iKuai_vmess-${suffix}${index}"
                name_node="iKuai_${name_node#iKuai_}"
                tls_val="false"
                [ "$tls" = "tls" ] && tls_val="true"

                yaml_config="name: $name_node, server: $server, port: $port, type: vmess, uuid: $uuid, alterId: ${alterId:-0}, cipher: ${cipher:-auto}, network: ${net:-tcp}, tls: $tls_val, skip-cert-verify: true, "
                if [ "$net" == "ws" ] && [ -n "$wsPath" ]; then
                    yaml_config+="ws-opts: {path: $wsPath, headers: {Host: $wsHost}}, "
                fi
                [ -n "$group" ] && yaml_config+="ik_group: $group, "
                yaml_config+="udp: true, tfo: $tfoEnable"
                printf "name:%s,|- {%s}\n" "$name_node" "$yaml_config" >>"$batch_file"
...
save_client() {
    ...
    if [ -n "$new_ip" ] && [ "$new_ip" != "$address_ip" ]; then
        old_ip="$address_ip"
        ...
        if [ "$redir_mod" = "混合模式" ]; then
            ...
            iptables -w -t nat -D shellcrash -p tcp -s $address_ip -j REDIRECT --to-ports 7892 >/dev/null 2>&1
            iptables -w -t mangle -D PREROUTING -p tcp -s $address_ip -j CONNMARK --restore-mark >/dev/null 2>&1
            iptables -w -t mangle -D shellcrash_mark -p udp -s $address_ip -j MARK --set-mark 7892 >/dev/null 2>&1
        else
            iptables -w -t nat -D shellcrash -p tcp -s $address_ip -j CONNMARK --set-mark 7899 >/dev/null 2>&1
            iptables -w -t nat -D shellcrash -p tcp -s $address_ip -j REDIRECT --to-ports 7892 >/dev/null 2>&1
            iptables -w -t mangle -D PREROUTING -p tcp -s $address_ip -j CONNMARK --restore-mark >/dev/null 2>&1
            iptables -w -t mangle -D shellcrash_mark -p tcp -s $address_ip -j MARK --set-mark 7892 >/dev/null 2>&1
            iptables -w -t mangle -D shellcrash_mark -p udp -s $address_ip -j MARK --set-mark 7892 >/dev/null 2>&1
        fi
        ...
        # 配置文件仅仅做新旧IP替换，后续代码会根据新IP继续更新对应记录的其它字段
        address_ip="$new_ip"
        new_ip_format="${address_ip}"
        [[ "$new_ip_format" != */* ]] && new_ip_format="${new_ip_format}/32"
        old_ip_format="${old_ip}"
        [[ "$old_ip_format" != */* ]] && old_ip_format="${old_ip_format}/32"
        
        sed -i "s|${old_ip_format}|${new_ip_format}|g" "$CRASHDIR/yamls/rules.yaml"
        sed -i "s|^${old_ip_format}\$|${new_ip_format}|" "$CRASHDIR/configs/ip_filter"
        sed -i "s|^${old_ip_format}\$|${new_ip_format}|" "$CRASHDIR/configs/disabled_ips" 2>/dev/null
    fi
...
# 白名单相关方法
save_whitelist() {
    local domainTxt=$(echo "$domainTxt64" | base64 -d)
    local ipTxt=$(echo "$ipTxt64" | base64 -d)

    mkdir -p $CRASHDIR/ruleset
    echo "$domainTxt" >$CRASHDIR/ruleset/direct_domains.txt
    echo "$ipTxt" >$CRASHDIR/ruleset/direct_ips.txt

    sed -i "/whitelistNode=/d" $ADV_SETTINGFILE
    echo "whitelistNode=$whitelist_node" >>$ADV_SETTINGFILE
    whitelistNode="$whitelist_node"
...
__show_domain_whitelist() {
    ...
    local whitelist_node=${whitelistNode:-"DIRECT"}
    json_append __json_result__ whitelist_node:str
}
__show_ip_whitelist() {
...
__show_resources() {
    ...
    if [ -s "$RESOURCE_CACHE_FILE" ]; then
        local resources=$(jq -c --arg code "$resellercode" '.resources |= map(select(.reseller == "*" or .reseller == $code))' "$RESOURCE_CACHE_FILE")
