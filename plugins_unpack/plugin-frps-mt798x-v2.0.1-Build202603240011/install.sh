...
install()
{
	...
	[ -f $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini ] || cp $INSTALL_DIR/bin/frps.ini $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini
	
	[ -f $INSTALL_DIR/bin/frps ] && chmod +x $INSTALL_DIR/bin/frps && ln -sf $INSTALL_DIR/bin/frps /usr/bin/frps
	...
	if [ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/autostart" ]; then
		(
			while true; do
				...
				[ $? -eq 0 ] && break
				debug "${PLUGIN_NAME}自动启动失败，30秒后重试！"
				sleep 30
			done
			debug "${PLUGIN_NAME}自动启动成功！"
