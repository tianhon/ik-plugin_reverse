...
start()
{
	[ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini" ] || return

	[ -d $EXT_PLUGIN_LOG_DIR ] || mkdir -p $EXT_PLUGIN_LOG_DIR
	echo "启动FRPS" > $EXT_PLUGIN_LOG_DIR/frps.log

	killall -q frps
	frps -c $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini >> $EXT_PLUGIN_LOG_DIR/frps.log 2>&1 &
...
save()
{
	local configpath=$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini
...
__show_config()
{
	...
	if [ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini" ]; then
		config=$(cat $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frps.ini | base64 | tr -d "\n")
	fi

	version=$(frps -v 2>/dev/null)
