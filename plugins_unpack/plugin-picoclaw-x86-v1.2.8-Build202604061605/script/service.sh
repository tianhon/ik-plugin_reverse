...
export CHROOT_ID="picoclaw"
PLUGIN_NAME="picoclaw"
CHROOTDIR=$(chrootmgt get_chroot_dir)
CLAWBINDIR=$EXT_PLUGIN_INSTALL_DIR/$PLUGIN_NAME/bin

start() {

    pidof picoclaw >/dev/null 2>&1 && return 0
    chrootmgt run "$CLAWBINDIR/picoclaw-launcher -public >/dev/null &"
    return 0
}

stop() {
    ! pidof picoclaw && ! pidof picoclaw-launcher && return 0
    killall picoclaw
    killall picoclaw-launcher
    return 0
}
...
__show_data() {
  ...
  pidof picoclaw-launcher >/dev/null 2>&1 && status=1
