...
install()
{
  ...
  # 首次安装或重新安装，则初始化配置文件
	if [ "$type" = "new" -o "$type" = "reinstall" ]; then
		...
		mkdir -p $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/configs
  ...
  cp -rf $INSTALL_DIR/bin/configs/* $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/configs/
  ...
  touch $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/yamls/rules.yaml
	...
	chrootmgt set_profile "export SAFE_PATHS='$EXT_PLUGIN_INSTALL_DIR/$PLUGIN_NAME/bin:/tmp/ShellCrash'" "$PLUGIN_NAME"
