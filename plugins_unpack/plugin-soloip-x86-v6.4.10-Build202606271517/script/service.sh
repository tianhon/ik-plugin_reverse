...
ensure_openresty_proxy() {
    local nginx_conf="/usr/openresty/conf/webman.conf"
    local tmp_conf="/tmp/soloip_webman.conf.$$"

    [ -f "$nginx_conf" ] || return 0

    if grep -q "# --- SoloIP Proxy Start ---" "$nginx_conf" 2>/dev/null; then
        if ! grep -q "# --- SoloIP Proxy End ---" "$nginx_conf" 2>/dev/null; then
            debug "OpenResty SoloIP 代理配置标记不完整，跳过自动替换"
            return 0
        fi
        if sed '/# --- SoloIP Proxy Start ---/,/# --- SoloIP Proxy End ---/d' "$nginx_conf" > "$tmp_conf" 2>/dev/null; then
        ...
        rm -f "$tmp_conf"
    elif grep -q "location /soloip/" "$nginx_conf" 2>/dev/null; then
        debug "OpenResty 已存在非托管 SoloIP 反向代理配置，跳过注入"
        return 0
    fi
...
location /soloip/ {
	...
	error_page 404 500 502 503 504 =200 /soloip-unavailable.html;
}

location = /soloip-unavailable.html {
	internal;
	default_type text/html;
	add_header Cache-Control "no-store";
	alias /usr/ikuai/www/plugins/soloip/unavailable.html;
}
# --- SoloIP Proxy End ---
EOF
