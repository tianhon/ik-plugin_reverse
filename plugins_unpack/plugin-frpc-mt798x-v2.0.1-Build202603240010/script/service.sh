...
start()
{
	[ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frpc.ini" ] || return

	[ -d $EXT_PLUGIN_LOG_DIR ] || mkdir -p $EXT_PLUGIN_LOG_DIR
	echo "启动FRPC" > $EXT_PLUGIN_LOG_DIR/frpc.log

	killall -q frpc
	frpc -c $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frpc.ini >> $EXT_PLUGIN_LOG_DIR/frpc.log 2>&1 &
	
	sleep 2
	if killall -q -0 frpc; then
		echo "启动FRPC成功" >> $EXT_PLUGIN_LOG_DIR/frpc.log
		return 0
	else
		echo "启动FRPC失败，请检查配置文件" >> $EXT_PLUGIN_LOG_DIR/frpc.log
		return 1
	fi
}
...
stop()
{
	killall -q frpc
	# 自启动锁死时可用停止按钮解锁
	if [ ! -f /tmp/iktmp/plugins/${PLUGIN_NAME}_installed ]; then
...
save()
{
	local configpath=$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frpc.ini
...
__show_config(){
	...
	if [ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frpc.ini" ]; then
		config=$(cat $EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/frpc.ini | base64 | tr -d "\n")
	fi

	version=$(frpc -v 2>/dev/null)
